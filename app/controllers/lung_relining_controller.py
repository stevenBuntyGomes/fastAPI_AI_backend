...
async def create_lung_relining_entry(data: LungReliningCreateRequest, user: dict):
    try:
        ...
        doc = entry.model_dump(by_alias=True, exclude_none=True)  # avoid "_id": None
...
async def update_lung_relining_last_relapse(entry_id: str, data: LungReliningUpdateRequest, user: dict):
    """
    Update only last_relapse_date for the given entry_id (owned by current user),
    and recompute delta_seconds and percent_of_90_days using the stored quit_date.
    """
    # Validate ObjectId
    try:
    ...
    # Recompute metrics
    delta_seconds = (new_last_utc - quit_dt_utc).total_seconds()
    percent_of_90_days = (delta_seconds / NINETY_DAYS_SECONDS) * 100
    ...
    update_doc = {
        ...
        "percent_of_90_days": float(percent_of_90_days),
    ...
    # Build response (current user info)
